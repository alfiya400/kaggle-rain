...
def calcStat(seq):
    ...
    if seq_not_null.size > 1:
        stat = [not_null.sum(), seq_not_null.mean(), seq_not_null.std(ddof=1), seq_not_null.min(),
                np.percentile(seq_not_null, 25), np.percentile(seq_not_null, 50), np.percentile(seq_not_null, 75), seq_not_null.max()]
    elif seq_not_null.size == 1:
        _ = seq_not_null[0]
        stat = [1, _, 0, _, _, _, _, _]
    else:
        stat = [np.nan] * 8
    return stat
...
STATS = [u'count', u'mean', u'std', u'min', u'25%', u'50%', u'75%', u'max']
...
def appendData(l, idx, h, columns, new_row, l_distance=None):
    ...
    if h == "HydrometeorType":
        ...
        values = l[idx]
        columns.extend([h + "_" + k for k in [u'min', u'50%', u'max']])
        new_row.extend([values.min(), np.percentile(values, 50), values.max()])
    elif h == "TimeToEnd":
        columns.append("time")
...
def transform(row, test=False):
    ...
    l = getValues(row["DistanceToRadar"])
    idx = np.insert(np.where(np.diff(l) != 0)[0], 0, 0)
    ...
    appendData(l, idx_, "DistanceToRadar", columns, new_row)
